...
print("🔄 双GPU集成验证")
print("="*70)
print(f"模型1 (Epoch 1) → GPU0")
print(f"模型2 (Epoch 2) → GPU1")
...
# 输出结果
print(f"\n{'='*70}")
print(f"📊 双GPU集成验证结果 (基于 {SAMPLE_SIZE} 样本)")
print(f"{'='*70}\n")

print(f"{'模型':<15} {'Precision':<12} {'Recall':<12} {'F1-Score':<12}")
print(f"{'-'*70}")
print(f"{'Epoch 1 (GPU0)':<15} {model1_p*100:<11.2f}% {model1_r*100:<11.2f}% {model1_f1*100:<11.2f}%")
print(f"{'Epoch 2 (GPU1)':<15} {model2_p*100:<11.2f}% {model2_r*100:<11.2f}% {model2_f1*100:<11.2f}%")
print(f"{'-'*70}")
...
with open("logs/ensemble_dual_gpu_result.json", 'w', encoding='utf-8') as f:
    json.dump(result, f, indent=2, ensure_ascii=False)

print(f"\n✅ 结果已保存至: logs/ensemble_dual_gpu_result.json")
