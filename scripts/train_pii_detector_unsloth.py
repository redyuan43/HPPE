...
try:
    ...
    print("请先安装: pip install unsloth")
    sys.exit(1)
...
# 训练配置
PER_DEVICE_TRAIN_BATCH_SIZE = 2  # 4B模型更大,减小到2
GRADIENT_ACCUMULATION_STEPS = 16  # 增大到16,保持effective_batch_size=32
NUM_TRAIN_EPOCHS = 3
...
def load_jsonl_as_dataset(path: str, desc: str = "数据") -> Dataset:
    """加载JSONL数据并转换为Unsloth格式"""
    print(f"\n加载{desc}: {path}")
...
train_dataset = load_jsonl_as_dataset(TRAIN_DATA, "训练数据")
eval_dataset = load_jsonl_as_dataset(VAL_DATA, "验证数据")
...
training_args = TrainingArguments(
    ...
    logging_steps = 10,
    optim = "adamw_8bit",  # Unsloth推荐的8bit优化器
    weight_decay = 0.01,
...
trainer = SFTTrainer(
    ...
    max_seq_length = MAX_SEQ_LENGTH,
    args = training_args,
    packing = False,  # 不使用packing (保持样本独立性)
)
...
# 可选: 保存合并后的16bit模型 (用于推理)
print(f"保存合并后的16bit模型到: {OUTPUT_DIR}/final_merged_16bit")
...
# 可选: 保存4bit GGUF格式 (用于llama.cpp)
# print(f"保存4bit GGUF格式...")
# model.save_pretrained_gguf(
#     f"{OUTPUT_DIR}/final_gguf",
#     tokenizer,
