...
class UltraFastEvaluator:
    ...
    def __init__(self, model_path: str, device: str = "auto", timeout_per_sample: int = 15):
        self.model_path = Path(model_path)
        self.device = self._setup_device(device)
        self.timeout = timeout_per_sample  # 每个样本最多15秒

        print(f"\n⚡ 超快速模型验证 (防卡死版)")
        print(f"=" * 70)
        print(f"模型路径: {self.model_path}")
        print(f"设备: {self.device}")
        print(f"超时设置: {self.timeout}秒/样本")
        print(f"=" * 70)
    ...
    def detect_pii_safe(self, text: str) -> Dict[str, Any]:
        """带超时保护的PII检测"""
        try:
            # 设置超时信号
            ...
            result = self._detect_pii_core(text)
    ...
    def _detect_pii_core(self, text: str) -> Dict[str, Any]:
        """核心PII检测逻辑"""
        # 格式化提示词
        prompt = (
            f"<|im_start|>system\n"
        ...
        inputs = self.tokenizer(
            ...
            return_tensors="pt",
            truncation=True,
            max_length=200  # 从256降到200
        ).to(self.device)

        # 生成 (更小的max_new_tokens)
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                max_new_tokens=64,  # 从128降到64!
                do_sample=False,
    ...
    def evaluate_on_subset(self, test_data_path: str, sample_size: int = 500):
        ...
        print(f"\n  开始推理 (单样本超时{self.timeout}秒)...")
        start_time = time.time()

        for i, sample in enumerate(tqdm(samples, desc="  处理中")):
            # 每100个样本显示一次平均速度
            if i > 0 and i % 100 == 0:
                avg_time = (time.time() - start_time) / i
                print(f"\n  [{i}/{len(samples)}] 平均速度: {avg_time:.2f}秒/样本")

            # 预测
            pred_result = self.detect_pii_safe(sample["input"])

            # 检查错误
            if "error" in pred_result:
                if pred_result["error"] == "timeout":
                    timeout_count += 1
                error_count += 1
                continue
    ...
    def print_results(self, metrics: Dict[str, Any]):
        ...
        print(f"  成功: {metrics['success']}, 失败: {metrics['errors']}, 超时: {metrics['timeouts']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="超快速模型验证(防卡死)")
    ...
    parser.add_argument("--timeout", type=int, default=15, help="单样本超时(秒)")

    args = parser.parse_args()

    # 创建评估器
    evaluator = UltraFastEvaluator(args.model, timeout_per_sample=args.timeout)
